...
class DATA:
    ...
    def read_img(self, filename):
        ...
        labimg = cv2.cvtColor(
            cv2.resize(img, (config.IMAGE_SIZE, config.IMAGE_SIZE)),
            cv2.COLOR_BGR2Lab)
        labimg_ori = cv2.cvtColor(img, cv2.COLOR_BGR2Lab)
        return np.reshape(labimg[:, :, 0],
                          (config.IMAGE_SIZE, config.IMAGE_SIZE, 1)), labimg[:,
                                                                      :,
                                                                      1:], img, labimg_ori[
                                                                                :,
                                                                                :,
    ...
    def generate_batch(self):
        ...
        labimg_oritList = np.asarray(
            labimg_oritList) / 255  # values between 0 and 1
